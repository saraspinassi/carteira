...
async function tryAutoLoad(){
  try{
    const [h,m,c] = await Promise.all([
      fetch('../data/historico.json').then(r=>{if(!r.ok) throw 0; return r.json();}),
      fetch('../data/metas.json').then(r=>{if(!r.ok) throw 0; return r.json();}),
      fetch('../data/configuracoes.json').then(r=>{if(!r.ok) throw 0; return r.json();}),
    ]);
